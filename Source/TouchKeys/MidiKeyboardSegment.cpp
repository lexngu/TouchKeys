...
XmlElement* MidiKeyboardSegment::getPreset() {
    ...
    segmentElement->addChildElement(new XmlElement(*(properties.createXml("Properties").get())));
...
void MidiKeyboardSegment::modeMonophonicHandler(MidiInput* source, const MidiMessage& message) {
    if(message.isNoteOn()) {
        ...
        if(midiOutputController_ != 0) {
            MidiMessage newMessage = MidiMessage::noteOn(outputChannelLowest_ + 1, message.getNoteNumber() + outputTransposition_, message.getVelocity());
            midiOutputController_->sendMessage(outputPortNumber_, newMessage);
        }
        ...
        if(midiOutputController_ != 0) {
            MidiMessage newMessage = MidiMessage::noteOff(outputChannelLowest_ + 1, message.getNoteNumber() + outputTransposition_, message.getVelocity());
            midiOutputController_->sendMessage(outputPortNumber_, newMessage);
        }
    }
}
